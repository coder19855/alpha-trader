...
import { normalizeStopLoss } from './stop-utils.js';
import {
  RrLabel,
...
function findCandleEndIndex(candles: FyersAPI.Candle[], asOfSec: number): number {
...
export function advanceCandleEndIndex(
  ...
  if (prevEnd < 0) {
    return findCandleEndIndex(candles, asOfSec);
  }
  ...
  while (end > 0 && candles[end][0] > asOfSec) {
    end -= 1;
  }
  return candles[end][0] <= asOfSec ? end : -1;
}

const IST_MINUTE_FORMATTER = new Intl.DateTimeFormat('en-GB', {
...
const IST_WEEKDAY_FORMATTER = new Intl.DateTimeFormat('en-GB', {
  timeZone: TIMELINE_DEFAULTS.IST_TIMEZONE,
  weekday: 'short',
});

function getIstMinutes(epochSec: number): number {
  const parts = IST_MINUTE_FORMATTER.formatToParts(new Date(epochSec * 1000));
...
export function isNseSession(epochSec: number): boolean {
  const day = IST_WEEKDAY_FORMATTER.format(new Date(epochSec * 1000));
...
export function summarizeTimelinePoints(
  ...
  endSpot: number,
): TechnicalAnalysisTimelineResponse['summary'] {
