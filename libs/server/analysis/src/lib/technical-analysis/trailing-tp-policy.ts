...
/** Signed R from entry (negative = adverse excursion). */
export function signedR(
  action: TradeAction,
  price: number,
  entry: number,
  risk: number,
  ...
  if (risk <= 0) return 0;
  if (action === 'CE-BUY') return +((price - entry) / risk).toFixed(3);
  if (action === 'PE-BUY') return +((entry - price) / risk).toFixed(3);
  return 0;
}

/**
 * Locked reversal floor in R-multiples.
 * BE lock at 1.0 / Tier locks at 1.0 (1.25 peak) / 1.5 (1.5 peak) / 2.5;
 * beyond 4R the floor ratchets (peak − giveback).
 */
export function resolveTrailFloorR(peakR: number): number | null {
  if (peakR < EARLY_BE_LOCK_R) return null;
  if (peakR < 1.25) return 0;
  if (peakR < 1.5) return 1.0;
...
export function formatTrailFloorHitLevel(floorR: number): RrLabel | 'TRAIL_FLOOR' | 'BE' {
  if (Math.abs(floorR) < 0.05) return 'BE';
  if (Math.abs(floorR - 1.0) < 0.08) return '1:1';
...
export function buildTrailingTpHoldGuidance(params: {
  ...
  if (params.trailing.floorBreached && params.trailing.lockedFloorR != null) {
    const isBe = params.trailing.lockedFloorR === 0;
    return {
      holdAdvice: 'exit',
      holdHeadline: isBe
        ? 'Reversal through Break-Even — protected capital after early extension.'
        : `Reversal through ${floorLabel} trail floor — book per ratchet plan.`,
      holdReasons: [
  ...
  if (params.trailing.peakR >= 1.0 && params.trailing.peakR < 1.25) {
    return {
      holdAdvice: 'hold',
      holdHeadline: 'Early extension (1R+) — Stop Loss moved to Break-Even.',
      holdReasons: [
        'Capital protected. Holding for 1.25R extension to lock 1R profit.',
        `Current ${params.currentR.toFixed(2)}R · Peak ${params.trailing.peakR.toFixed(1)}R.`,
        ...reasons,
      ],
      alertKind: 'HOLD_REVIEW',
    };
  ...
  if (params.trailing.peakR >= 1.25 && params.trailing.peakR < 1.5) {
    const canHold = params.conviction >= params.enterThreshold;
    return {
      holdAdvice: canHold ? 'partial' : 'exit',
      holdHeadline: canHold
        ? '1:1 profit locked — book partial, trail rest toward 1:1.5 / 1:2.5.'
        : '1:1 profit locked but conviction weakened — prefer booking gains.',
      holdReasons: canHold
        ? [
            'Floor locked at 1R after 1.25R extension — provides breathing room.',
            `Next engine target is ${params.nextTpRr ?? '1:1.5'}.`,
            ...reasons,
          ]
        : [
            '1R profit locked but follow-through quality is poor.',
            ...reasons,
          ],
      alertKind: 'HOLD_REVIEW',
    };
