...
export interface DecisionEngineOptions {
  ...
  /** Penalize or block entries after the move has already extended. */
  chaseDecay?: boolean;
}
...
export default fp(
  async (fastify: FastifyInstance) => {
    ...
    function computeConfluentDecision(
      ...
      let chaseDecayResult: ChaseDecayResult | undefined;
      let chaseEntryBlocked = false;
      if (options?.chaseDecay && tradeDirection !== 'neutral') {
        const recentMom =
          primaryTF === '5m'
            ? priceMomentum.recent?.['5m']
            : primaryTF === '1h'
              ? priceMomentum.recent?.['1h']
              : priceMomentum.recent?.['15m'];
        chaseDecayResult = evaluateChaseDecay({
          action: tradeDirection === 'bullish' ? 'CE-BUY' : 'PE-BUY',
          primaryScore,
          recentMomentum: recentMom,
          tradeSetup: price.tradeSetup,
        });
        if (chaseDecayResult.blocked) {
          chaseEntryBlocked = true;
          pushBonus('Chase block (late entry)', -Math.round(blended));
        } else if (chaseDecayResult.decayPercent > 0) {
          const before = Math.round(blended);
          blended *= 1 - chaseDecayResult.decayPercent;
          pushBonus(
            `Chase decay (${chaseDecayResult.extensionR.toFixed(1)}R ext)`,
            Math.round(blended) - before,
          );
        }
      }
