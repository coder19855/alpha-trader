...
export default fp(
  async (fastify: FastifyInstance) => {
    ...
    const getConfluentTradeSignal = (params: {
      ...
      entryVetoMode?: import('@alpha-trader/server-shared').VetoMode;
      tradingStyle: TradingStyle;
    ...
    const calculateRSI = (candles: FyersAPI.Candle[], period = 14): number => {
      if (candles.length < period + 1) return 50;

      const close = candles.map((c) => c[4]);
      const rsiValues = RSI.calculate({ period, values: close });
      const last = rsiValues[rsiValues.length - 1];
      return Number.isFinite(last) ? last : 50;
    };

    const calculateMACDHistogram = (
      candles: FyersAPI.Candle[],
      fastPeriod = 12,
      slowPeriod = 26,
      signalPeriod = 9,
    ): number => {
      if (candles.length < slowPeriod + signalPeriod) return 0;
    ...
    const calculateEMATrend = (
      ...
      const close = candles.map((c) => c[4]);
      const emaValues = EMA.calculate({ period, values: close });
      const lastEma = emaValues[emaValues.length - 1];
      const lastClose = close[close.length - 1];
      if (!Number.isFinite(lastEma) || !Number.isFinite(lastClose)) return 0;
      if (lastClose > lastEma) return 1;
      if (lastClose < lastEma) return -1;
      return 0;
    };

    const calculateBollingerSignal = (
      candles: FyersAPI.Candle[],
      period = 20,
      stdDev = 2,
    ): number => {
      ...
      const close = candles.map((c) => c[4]);
      const bands = BollingerBands.calculate({ period, values: close, stdDev });
      const last = bands[bands.length - 1];
      const lastClose = close[close.length - 1];
      if (!last || !Number.isFinite(lastClose)) return 0;
      if (lastClose > last.upper) return 1;
      if (lastClose < last.lower) return -1;
      return 0;
    };
