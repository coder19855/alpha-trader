...
import { Timeframe } from '@alpha-trader/server-shared';
...
export function buildPriceActionComponentGauges(
  ...
  const structureHint =
    'Score −1 (bearish) → 0 (flat) → +1 (bullish). Composite of structure, breakout, ATR/ADX, momentum, FVG/OB.';
  ...
  for (const key of PA_ORDER) {
    ...
    let interpretation = comp.explanation;

    if (key === '5m' || key === '15m' || key === '1h') {
      readout = `${value >= 0 ? '+' : ''}${value.toFixed(2)}`;
      interpretation = interpretation ?? structureHint;
    }
    if (key === 'mtfScore') {
      readout = `${value >= 0 ? '+' : ''}${value.toFixed(2)}`;
      interpretation =
        interpretation ??
        'Weighted blend: 0.2×5m + 0.3×15m + 0.5×1h. −1 weak bearish stack, +1 strong bullish stack.';
    }
    if (key === 'alignment') {
      ...
      interpretation =
        interpretation ??
        `Gauge maps count to needle: −1 = 0/3 TFs share primary direction, 0 ≈ 1–2/3, +1 = 3/3. Readout is the raw count (${aligned}/3). Mild scores still count if signs match.`;
    }
    if (key === 'higherTFConfirmation') {
      ...
      interpretation =
        interpretation ??
        'Whether 1h structure confirms primary TF. Separate from 1h structure score and TF alignment count.';
    }
